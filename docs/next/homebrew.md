...
Do not commit machine-specific `file://` URLs, binary archives, Homebrew receipts, or shell backups. A shared formula needs an established release location and checksums for actual published artifacts. Until those exist, there is no portable tap configuration to add.
