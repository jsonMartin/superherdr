...
- **macOS:** built on Apple Silicon with `MACOSX_DEPLOYMENT_TARGET=13.0`. The 0.9.0.1 release needed Homebrew's patched `zig@0.15` plus the macOS 26.5 SDK because stock Zig 0.15.2 could not link against the newer SDKs; Zig 0.16 has not been exercised against the macOS 26.5/27 SDKs yet, so verify linking (the version smoke test and a PTY roundtrip) before publishing. The Homebrew bottle is assembled from the archive's binary, not produced by `brew bottle`.
