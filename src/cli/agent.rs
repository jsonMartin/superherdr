...
fn agent_explain(args: &[String]) -> std::io::Result<i32> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!("usage: superherdr agent explain <target> [--json|--verbose]");
                eprintln!(
                    "usage: superherdr agent explain --file PATH --agent LABEL [--json|--verbose]"
                );
                return Ok(0);
            }
            ...
            value => {
                if target.is_some() {
                    eprintln!("usage: superherdr agent explain <target> [--json]");
                    return Ok(2);
                }
    ...
    let explain = if let Some(path) = file {
        if target.is_some() {
            eprintln!("usage: superherdr agent explain --file PATH --agent LABEL [--json]");
            return Ok(2);
        }
        let Some(agent_label) = agent else {
            eprintln!("superherdr agent explain --file requires --agent LABEL");
            return Ok(2);
        };
...
fn agent_start(args: &[String]) -> std::io::Result<i32> {
    let Some(name) = args.first() else {
        eprintln!("usage: superherdr agent start <name> --kind KIND --pane ID [--timeout MS] [-- <agent-args...>]");
        return Ok(2);
    };
    let separator = args
        .iter()
...
fn agent_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: superherdr agent list");
        return Ok(2);
    }
...
fn agent_get(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: superherdr agent get <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: superherdr agent get <target>");
...
fn agent_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: superherdr agent focus <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: superherdr agent focus <target>");
...
fn agent_attach(args: &[String]) -> std::io::Result<i32> {
    let (target, takeover) = match super::parse_attach_target(
        args,
        "usage: superherdr agent attach <target> [--takeover]",
    ) {
        Ok(parsed) => parsed,
...
fn agent_wait(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: superherdr agent wait <target> [--until STATUS]... [--timeout MS]");
        return Ok(2);
    };
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!(
                    "usage: superherdr agent wait <target> [--until STATUS]... [--timeout MS]"
                );
                return Ok(0);
            }
            other => {
...
fn agent_rename(args: &[String]) -> std::io::Result<i32> {
    let [target, value] = args else {
        eprintln!("usage: superherdr agent rename <target> <name>|--clear");
        return Ok(2);
    };
...
fn agent_prompt(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!(
            "usage: superherdr agent prompt <target> <text> [--wait] [--until STATUS]... [--timeout MS]"
        );
        return Ok(2);
    };
...
fn agent_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: superherdr agent send-keys <target> <key> [key ...]");
        return Ok(2);
    }
...
fn agent_read(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: superherdr agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
        return Ok(2);
    };
...
fn print_agent_help() {
    eprintln!("superherdr agent commands:");
    eprintln!("  superherdr agent list");
    eprintln!("  superherdr agent get <target>");
    eprintln!("  superherdr agent read <target> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!("  superherdr agent send-keys <target> <key> [key ...]");
    eprintln!(
        "  superherdr agent prompt <target> <text> [--wait] [--until STATUS]... [--timeout MS]"
    );
    eprintln!("  superherdr agent rename <target> <name>|--clear");
    eprintln!("  superherdr agent focus <target>");
    eprintln!("  superherdr agent wait <target> [--until STATUS]... [--timeout MS]");
    eprintln!("  superherdr agent attach <target> [--takeover]");
    eprintln!(
        "  superherdr agent start <name> --kind KIND --pane ID [--timeout MS] [-- <agent-args...>]"
    );
    eprintln!("  superherdr agent explain <target> [--json|--format text|json] [--verbose]");
    eprintln!(
        "  superherdr agent explain --file PATH --agent LABEL [--json|--format text|json] [--verbose]"
    );
