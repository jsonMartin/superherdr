...
fn tab_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: superherdr tab get <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: superherdr tab get <tab_id>");
...
fn tab_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: superherdr tab focus <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: superherdr tab focus <tab_id>");
...
fn tab_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: superherdr tab rename <tab_id> <label>");
        return Ok(2);
    }
...
fn tab_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: superherdr tab close <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: superherdr tab close <tab_id>");
...
fn print_tab_help() {
    eprintln!("superherdr tab commands:");
    eprintln!("  superherdr tab list [--workspace <workspace_id>]");
    eprintln!(
        "  superherdr tab create [--workspace <workspace_id>] [--cwd PATH] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]"
    );
    eprintln!("  superherdr tab get <tab_id>");
    eprintln!("  superherdr tab focus <tab_id>");
    eprintln!("  superherdr tab rename <tab_id> <label>");
    eprintln!("  superherdr tab close <tab_id>");
}
