...
fn workspace_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: superherdr workspace list");
        return Ok(2);
    }
...
fn workspace_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: superherdr workspace get <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: superherdr workspace get <workspace_id>");
...
fn workspace_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: superherdr workspace focus <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: superherdr workspace focus <workspace_id>");
...
fn workspace_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: superherdr workspace rename <workspace_id> <label>");
        return Ok(2);
    }
...
fn workspace_report_metadata(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: superherdr workspace report-metadata <workspace_id> --source ID [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
        return Ok(2);
    };
...
fn workspace_close(args: &[String]) -> std::io::Result<i32> {
    let (raw_workspace_id, close_group) = match args {
        ...
        _ => {
            eprintln!("usage: superherdr workspace close <workspace_id> [--group]");
            return Ok(2);
        }
    };
...
fn print_workspace_help() {
    eprintln!("superherdr workspace commands:");
    eprintln!("  superherdr workspace list");
    eprintln!("  superherdr workspace create [--cwd PATH] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]");
    eprintln!("  superherdr workspace get <workspace_id>");
    eprintln!("  superherdr workspace focus <workspace_id>");
    eprintln!("  superherdr workspace rename <workspace_id> <label>");
    eprintln!("  superherdr workspace report-metadata <workspace_id> --source ID [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
    eprintln!("  superherdr workspace close <workspace_id> [--group]");
}
