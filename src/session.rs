...
pub fn configure_from_args(args: &[String]) -> Result<Vec<String>, String> {
        ...
        && args.get(2).map(String::as_str) == Some("attach")
    {
        ...
        let Some(name) = args.get(3) else {
            return Err("usage: superherdr session attach <name>".to_string());
        };
        if args.len() != 4 {
            return Err("usage: superherdr session attach <name>".to_string());
        }
...
pub fn local_attach_command() -> String {
    match active_name() {
        Some(name) => format!("superherdr session attach {name}"),
        None => "superherdr".to_string(),
    }
}
...
pub fn stop_command_for(name: Option<&str>) -> String {
    match name {
        Some(name) => format!("superherdr session stop {name}"),
        None => "superherdr server stop".to_string(),
    }
}
...
mod tests {
    ...
    fn configure_from_args_removes_global_session_option() {
        ...
        assert_eq!(cleaned, vec!["superherdr", "workspace", "list"]);
        std::env::remove_var(SESSION_ENV_VAR);
        clear_explicit_session_for_test();
    }
    ...
    fn configure_from_args_accepts_equals_form() {
        ...
        assert!(explicit_session_requested());
        assert_eq!(cleaned, vec!["superherdr", "server", "stop"]);
        std::env::remove_var(SESSION_ENV_VAR);
        clear_explicit_session_for_test();
    }
    ...
    fn local_attach_command_uses_default_launch_for_default_session() {
        ...
        assert_eq!(local_attach_command(), "superherdr");
    }
    ...
    fn local_attach_command_uses_session_attach_for_named_session() {
        ...
        assert_eq!(local_attach_command(), "superherdr session attach work");
    ...
    fn local_stop_command_uses_server_stop_for_default_session() {
        ...
        assert_eq!(local_stop_command(), "superherdr server stop");
    ...
    fn local_stop_command_uses_session_stop_for_named_session() {
        ...
        assert_eq!(local_stop_command(), "superherdr session stop work");
    ...
    fn restart_after_update_guidance_names_stop_and_attach_commands() {
        assert_eq!(
            restart_after_update_guidance(
                "superherdr session stop work",
                Some("superherdr session attach work")
            ),
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `superherdr session stop work`, then run `superherdr session attach work` again."
        );
    }
    ...
    fn active_restart_after_update_guidance_respects_socket_override() {
        ...
        assert_eq!(
            ...
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `HERDR_SOCKET_PATH=/tmp/custom-herdr.sock superherdr server stop`, then restart Herdr with the same socket override."
        );
