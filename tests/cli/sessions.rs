...
fn dead_server_cli_reports_one_session_aware_json_line() {
    fn assert_server_not_running(
        ...
        assert_eq!(
            ...
            format!(
                "no superherdr server is running at {}; run `{attach_command}` to start or attach it",
                socket_path.display()
            )
        );
    }
    ...
    assert_server_not_running(missing, &named_socket, "superherdr session attach foo");

    let stale_socket = runtime_dir.join("stale.sock");
    drop(UnixListener::bind(&stale_socket).unwrap());
    let stale = Command::new(env!("CARGO_BIN_EXE_superherdr"))
        ...
        .output()
        .unwrap();
    assert_server_not_running(stale, &stale_socket, "superherdr");
...
fn integration_commands_run_locally_when_server_is_missing() {
    ...
    let workspace_list = Command::new(env!("CARGO_BIN_EXE_superherdr"))
        .args(["workspace", "list"])
    ...
    let integration_install = Command::new(env!("CARGO_BIN_EXE_superherdr"))
    ...
    let integration_status = Command::new(env!("CARGO_BIN_EXE_superherdr"))
    ...
    let integration_uninstall = Command::new(env!("CARGO_BIN_EXE_superherdr"))
...
fn integration_status_outdated_only_prints_action_for_legacy_install() {
    ...
    assert!(stderr.contains("installed superherdr integrations need updating"));
    assert!(stderr.contains("superherdr integration install pi"));
