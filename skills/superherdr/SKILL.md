---
name: superherdr
description: "Control Superherdr, a terminal multiplexer for coding agents. Use only when the user explicitly mentions Superherdr or asks to use Superherdr to inspect or control panes, tabs, workspaces, commands, or another agent. Do not use merely because a task could benefit from a background terminal, delegation, or parallel work. Requires HERDR_ENV=1."
---

# Superherdr

Superherdr is a terminal project built on Herdr. The inherited `HERDR_*` environment and API identifiers are intentional compatibility contracts.

Superherdr organizes terminals into workspaces, tabs, and panes, recognizes coding agents running inside panes, and exposes the current session through the `superherdr` CLI.

Before issuing any control command, verify that this agent is running inside a Superherdr-managed pane:

```bash
test "${HERDR_ENV:-}" = 1
```

If the check fails, say that you are not running inside Superherdr and stop. Do not inspect or control the focused Superherdr session from outside Superherdr.

When the check passes, the `superherdr` binary in `PATH` talks to the current session. Use it to inspect neighboring work, create terminal layout, start agents and commands, read output, and wait for state changes.
...
superherdr agent
superherdr pane
superherdr workspace
superherdr tab
superherdr worktree
superherdr terminal
superherdr notification
superherdr integration
superherdr session
superherdr machine
```

Do not run bare `superherdr` for discovery; it launches or attaches the TUI. Do not probe a mutating nested command by omitting arguments. Commands such as `superherdr workspace create` are valid with defaults and will execute.
...
A pane exists whether or not it contains an agent. `agent start` requires an existing available shell pane and never creates, splits, or moves layout. Use pane commands for ordinary processes. Use agent commands when Superherdr must validate agent identity or interpret `idle`, `working`, `blocked`, `done`, and `unknown` lifecycle states.
...
Superherdr injects the caller's context into each managed pane:
...
```bash
superherdr workspace list
superherdr tab list --workspace "$HERDR_WORKSPACE_ID"
superherdr pane current --current
superherdr pane list --workspace "$HERDR_WORKSPACE_ID"
superherdr agent list
```
...
```bash
superherdr pane layout --pane "$HERDR_PANE_ID"
```
...
```bash
superherdr pane split --current --direction right --cwd "$PWD" --no-focus
```
...
```bash
superherdr agent start reviewer --kind codex --pane <returned-pane-id>
```

Use the kind requested by the user. Run `superherdr agent` to inspect the installed kind list and options. Pass native agent arguments only after `--`:

```bash
superherdr agent start reviewer --kind codex --pane <returned-pane-id> -- <agent-args...>
```

A successful `agent start` returns only after Superherdr detects the expected agent in the same pane and considers it ready for interactive input. If the agent is blocked during startup, the command returns `agent_not_ready` immediately but keeps the name available for `agent read` and `agent send-keys`. Wait until the agent becomes idle before prompting it. Startup defaults to a 30-second timeout.

Submit work through the agent surface:

```bash
superherdr agent prompt reviewer "Review the current diff and report only actionable findings." --wait --timeout 120000
```
...
```bash
superherdr agent send-keys reviewer esc
superherdr agent send-keys reviewer ctrl+c
```

Superherdr validates all keys before writing any bytes. Read the result through the resolved agent:

```bash
superherdr agent get reviewer
superherdr agent read reviewer --source recent-unwrapped --lines 120
```
...
```bash
superherdr pane run <returned-pane-id> "just test"
superherdr pane wait-output <returned-pane-id> --match "test result" --timeout 120000
superherdr pane read <returned-pane-id> --source recent-unwrapped --lines 120
```
...
`--lines` asks Superherdr for more rows from the pane's available screen and host scrollback. If increasing it does not reveal more of a completed response, the pane is probably running the agent on the terminal's alternate screen. Rows that leave the alternate screen do not enter Superherdr's host scrollback, so a larger line count cannot recover them.
...
- Client and server versions can differ after an update. Check `superherdr status` before relying on new server features. A missing method is not permission to stop or upgrade a server.
- Never run `superherdr server stop` from an active session unless the user explicitly intends to stop the server and its pane processes.
- Never kill the main Superherdr process. Use named test sessions for experiments that need an isolated server.
